...
def main():
    ...
    model_D1 = FCDiscriminator(num_classes=args.num_classes)  # day判别器
    model_D1.train()
    model_D1.to(device)

    model_D2 = FCDiscriminator(num_classes=args.num_classes)  # night判别器
    model_D2.train()
    model_D2.to(device)



    recoverdir = "./snapshots/PSPNet"
    recoveriter = 19000
    dannetparm = torch.load(os.path.join(recoverdir, "dannet"+str(recoveriter))+".pth")
    dannetlightparm=torch.load(os.path.join(recoverdir,"dannet_light"+str(recoveriter))+".pth")
    dannetd1parm=torch.load(os.path.join(recoverdir,"dannet_d1_"+str(recoveriter))+".pth")
    dannetd2parm=torch.load(os.path.join(recoverdir,"dannet_d2_"+str(recoveriter))+".pth")

    model.load_state_dict(dannetparm)
    lightnet.load_state_dict(dannetlightparm)
    model_D1.load_state_dict(dannetd1parm)
    model_D2.load_state_dict(dannetd2parm)
    ...
    weights = torch.log(torch.FloatTensor([0.36869696, 0.06084986, 0.22824049, 0.00655399, 0.00877272, 0.01227341,
                                           ...
                                           0.00413907])).cuda()  # category在label中的占比，仅对cityscapes有效！
    ...
    static_loss = StaticLoss(num_classes=11, weight=weights[:11])  # 相似时：仅有静态物体相似。后面几个都是动态物体，不考虑
    ...
    for i_iter in range(recoveriter,args.num_steps):
        loss_seg_value = 0
        ...
        for sub_i in range(args.iter_size):  # 1
            # train G
            ...
            D_out_d = model_D1(F.softmax(pred_target_d, dim=1))  # dim1上是0-19的19个分类.F部分是分类结果，总部分是判别结果
            D_label_d = torch.FloatTensor(D_out_d.data.size()).fill_(source_label).to(device)
            loss_adv_target_d = weightedMSE(D_out_d,
                                            D_label_d)  # softmax得到的每个类别的概率，与全0作MSE。对抗损失：生成的day图片让判别器无法识别（无法识别即为所有类别的概率相等）
            ...
            psudo_prob = torch.zeros_like(pred_target_d)  # day来的label源
            threshold = torch.ones_like(pred_target_d[:, :11, :, :]) * 0.2
            threshold[pred_target_d[:, :11, :,
                      :] > 0.4] = 0.8  # 通过对day图像判断：静态物体的绝对概率够高的话就认为“该像素更可能是静态物体而不是动态物体”，给予更高的“label中占比”
